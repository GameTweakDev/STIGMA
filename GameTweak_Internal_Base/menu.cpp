...
void Menu::Aimbot_Tab()
{
	gui->begin_group();
	{
	    ...
	    {
			gui->checkbox("Aimbot", &g_settings.Mods.Aimbot);
		}
	    gui->end_child();
...
void Menu::Player_ESP_Tab()
{
	gui->begin_group();
	{
	    ...
	    gui->begin_child("A", "Visual Elements", ImVec2(panelWidth, 0));
	    {
			...
			gui->checkbox("Chams", &g_settings.CHAMS.Chams_Player);
...
void Menu::Item_ESP_Tab()
{
	gui->begin_group();
	{
		...
		{
			gui->checkbox("Stone", &g_settings.WorldESP.StoneNode);
			gui->checkbox("Metal", &g_settings.WorldESP.MetalNode);
			gui->checkbox("Sulfur", &g_settings.WorldESP.SulfurNode);
			gui->checkbox("Hemp", &g_settings.WorldESP.Hemp);
			gui->checkbox("Sleepers", &g_settings.WorldESP.Sleepers);
		}
		gui->end_child();
		...
		{
			gui->checkbox("Sleeping Bags", &g_settings.WorldESP.SleepingBags);
			gui->checkbox("Vehicles", &g_settings.WorldESP.Vehicles);
			gui->checkbox("Barrels", &g_settings.WorldESP.Barrels);
			gui->checkbox("Tool Cupboards", &g_settings.WorldESP.ToolCupboards);
			gui->checkbox("Death Bags", &g_settings.WorldESP.DeathBags);
		}
		gui->end_child();
	}
	gui->end_group();
...
void Menu::Local_Player_Tab()
{
	gui->begin_group();
	{
	    ...
	    // Left panel - GamePlay Mods
	    gui->begin_child("A", "GamePlay", ImVec2(panelWidth, 0));
	    {
			gui->checkbox("No Recoil", &g_settings.Mods.NoRecoil);
			gui->checkbox("No Spread", &g_settings.Mods.NoSpread);
			gui->checkbox("Easy Farm", &g_settings.Mods.QuickFarm);

			gui->slider_float("Bullet/Arrow Velocity", &g_settings.Mods.Bullet_Velocity, 1000.0f, 20000.0f, "%.0f");
			gui->slider_float("Arrow Scale", &g_settings.Mods.Bullet_Scale, 0.10f, 2.0f, "%.2f");
			//// Combat related mods
