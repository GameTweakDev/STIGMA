...
unsigned char ImGui_vertexShaderDX11_11_1[] = {
 ...
 unsigned char ImGui_pixelShaderDX11_11_1[] = {
 ...
 bool ImGui_ImplDX11_CreateDeviceObjects()
 {
     ...
     // Check feature level and log it
     D3D_FEATURE_LEVEL featureLevel = bd->pd3dDevice->GetFeatureLevel();

     // Log human-readable feature level
     ...
     case D3D_FEATURE_LEVEL_12_0: featureLevelStr = "12.0"; break;
     case D3D_FEATURE_LEVEL_12_1: featureLevelStr = "12.1"; break;
     case D3D_FEATURE_LEVEL_12_2: featureLevelStr = "12.2"; break;
     }

     // Create the vertex shader from pre-compiled bytecode
     {

         HRESULT hr = bd->pd3dDevice->CreateVertexShader(ImGui_vertexShaderDX11_11_1, sizeof(ImGui_vertexShaderDX11_11_1), nullptr, &bd->pVertexShader);
         if (FAILED(hr)) {
             return false;
         }
         ...
         // IMPORTANT FIX: Use VERTEX shader for input layout, not pixel shader!
         hr = bd->pd3dDevice->CreateInputLayout(local_layout, 3, ImGui_vertexShaderDX11_11_1, sizeof(ImGui_vertexShaderDX11_11_1), &bd->pInputLayout);
         if (FAILED(hr)) {
             return false;
         }
     ...
     // Create the pixel shader from pre-compiled bytecode
     // Choose shader based on feature level
     {
         HRESULT hr;
         const unsigned char* pixelShaderData = nullptr;
         size_t pixelShaderSize = 0;

         if (featureLevel >= D3D_FEATURE_LEVEL_11_0) {
             pixelShaderData = ImGui_pixelShaderDX11_11_1;
             pixelShaderSize = sizeof(ImGui_pixelShaderDX11_11_1);
         }
         else if (featureLevel >= D3D_FEATURE_LEVEL_10_0) {
             // Add your SM 4.0 shader here if you have one
             // For now, try the 11.1 shader (might be backward compatible)
             pixelShaderData = ImGui_pixelShaderDX11_11_1;
             pixelShaderSize = sizeof(ImGui_pixelShaderDX11_11_1);
         }
         else {
         ...
         hr = bd->pd3dDevice->CreatePixelShader(pixelShaderData, pixelShaderSize, nullptr, &bd->pPixelShader);
         if (FAILED(hr)) {
             return false;
         }
     }
