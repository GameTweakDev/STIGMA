...
{
	//std::cout << "Kiero init called with render type: " << _renderType << "\n";

	if (g_renderType != RenderType::None)
	{
		//std::cout << "Kiero already initialized with render type: " << g_renderType << "\n";
	...
	if (_renderType != RenderType::None)
	{
		...
		{
			//std::cout << "Attempting to create temporary window...\n";
			...
			{
#if KIERO_INCLUDE_D3D11
				//std::cout << "Checking for D3D11 module...\n";
				HMODULE libD3D11;
				if ((libD3D11 = ::GetModuleHandle(KIERO_TEXT("d3d11.dll"))) == NULL)
				{
					//std::cout << "d3d11.dll not found!\n";
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::ModuleNotFoundError;
				}
				//std::cout << "D3D11 module found at: " << libD3D11 << "\n";

				void* D3D11CreateDeviceAndSwapChain;
				if ((D3D11CreateDeviceAndSwapChain = ::GetProcAddress(libD3D11, "D3D11CreateDeviceAndSwapChain")) == NULL)
				{
					//std::cout << "Failed to get D3D11CreateDeviceAndSwapChain address\n";
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::UnknownError;
				}
				//std::cout << "Found D3D11CreateDeviceAndSwapChain at: " << D3D11CreateDeviceAndSwapChain << "\n";

				D3D_FEATURE_LEVEL featureLevel;
				const D3D_FEATURE_LEVEL featureLevels[] = { D3D_FEATURE_LEVEL_10_1, D3D_FEATURE_LEVEL_11_0 };

				//std::cout << "Setting up swap chain description...\n";
				DXGI_RATIONAL refreshRate;
				refreshRate.Numerator = 60;
				refreshRate.Denominator = 1;
				...
				//std::cout << "Attempting to create device and swap chain...\n";
				IDXGISwapChain* swapChain;
				ID3D11Device* device;
				...
				HRESULT hr = ((long(__stdcall*)(
					...
					ID3D11DeviceContext**))(D3D11CreateDeviceAndSwapChain))(
						NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, 0, featureLevels, 1, D3D11_SDK_VERSION,
						&swapChainDesc, &swapChain, &device, &featureLevel, &context);

				if (FAILED(hr))
				{
					//std::cout << "Failed to create device and swap chain! HRESULT: " << std::hex << hr << "\n";
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::UnknownError;
				}

				//std::cout << "Successfully created device and swap chain!\n";
				//std::cout << "Setting up methods table...\n";
