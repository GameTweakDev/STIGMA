...
class module : public range {
  public:
    ...
    explicit module(std::wstring_view name) noexcept
        : range(address_t{}, 0), m_base_name(name), m_full_path(name), m_loaded(false) {
        try_get_module();
    }
     ...
     */
    explicit module(std::string_view name) noexcept : module(to_wide(name)) {}

    /**
     * @brief Initialize a module wrapper for the main executable
     */
    module() noexcept :range(address_t{}, 0), m_loaded(false) {
        try_get_module();
    }
    ...
    [[nodiscard]] void* get_import(std::string_view symbol_name) const noexcept {
        ...
        auto* imports =
            m_base.at_offset(import_dir.VirtualAddress).cast<IMAGE_IMPORT_DESCRIPTOR*>();
        ...
        for (; imports->Name != 0; ++imports) {
            ...
            for (; thunk->u1.AddressOfData != 0; ++thunk, ++func) {
                ...
                auto* import =
                    m_base.at_offset(thunk->u1.AddressOfData).cast<IMAGE_IMPORT_BY_NAME*>();
  ...
  private:
    ...
    bool try_get_module() noexcept {
        ...
        for (auto* entry = head->Flink; entry != head; entry = entry->Flink) {
            ...
            if (std::equal(m_base_name.begin(), m_base_name.end(), current_name.begin(),
                           current_name.end(),
